  ...
  return (
    <header className={`navbar ${scrolled ? "scrolled" : "not-scrolled"}`}>
      <div className="inner w-full flex items-center justify-between">
        <a href="#hero" className="logo">Brandy.dev</a>
        ...
        <div className="hidden lg:flex items-center gap-4">
          ...
          <div className="language-btns">
            <button
              ...
              className={`language-btn group ${language === "es" ? "active" : ""}`}
            >
              <div className="inner"><span>ES</span></div>
            </button>
            <button
              onClick={() => setLanguage("en")}
              className={`language-btn group ${language === "en" ? "active" : ""}`}
            >
              <div className="inner"><span>EN</span></div>
            </button>
          </div>
        </div>
      </div>
      ...
      {menuOpen && (
        <div className="lg:hidden absolute top-0 left-0 w-full h-screen bg-black bg-opacity-95 flex flex-col items-center justify-center gap-10 z-40 transition-all">
          ...
          <div className="flex gap-4">
            <button onClick={() => setLanguage("es")} className={`text-white ${language === "es" && "underline"}`}>
              ES
            </button>
            <button onClick={() => setLanguage("en")} className={`text-white ${language === "en" && "underline"}`}>
              EN
            </button>
          </div>
        </div>
